...
function addAlarm() {
  ...
  li.innerHTML = `${label} - ${time} ${
    repeat ? "(Daily)" : ""
  } <button onclick="removeAlarm(this)">❌</button>`;
...
setInterval(() => {
  ...
  // Loop through a copy to safely modify alarmList
  alarmList.slice().forEach((alarm, index) => {
    if (
      alarm.time === currentTime &&
      (!currentAudio || currentAudio.ended)
    ) {
      stopCurrentAlarm(); // stop any previous sound

      // Play alarm
      currentAudio = alarm.audioURL
        ? new Audio(alarm.audioURL)
        : new Audio(document.getElementById("defaultBeep").src);
      ...
      // Remove from UI and list if not repeating
      if (!alarm.repeat) {
